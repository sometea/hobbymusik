...
export function Release({ title, artist, year, cover, slug, type, linkPrefix = 'releases' }: GeneralisedReleaseProps) {
  ...
  return <div className="flex flex-col items-center w-full lg:w-1/3 p-2">
    ...
    <div className="text-sm pt-2 w-full text-right border-t-2 border-black font-semibold">{year.getFullYear()}</div>
    {type && <div className="text-sm mb-8 lg:mb-16 w-full text-right font-semibold">{type}</div>}
  </div>;
}
